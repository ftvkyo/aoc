...
let patterns : pattern list =
  let pattern_base = [ 'A', (0, 0); 'M', (-1, -1); 'M', (-1, 1); 'S', (1, -1); 'S', (1, 1) ] in
  let rotate_90 (c, (x, y)) = c, (y, -x) in
  let transforms = [ id; rotate_90; compose rotate_90 rotate_90; compose rotate_90 @@ compose rotate_90 rotate_90 ] in
...
let solve (input : string array) : string =
  ...
  let count_at x acc y _e = acc + if List.exists (check_pattern @@ get x y) patterns then 1 else 0 in
  let count_at_x acc x row = acc + (Seq.fold_lefti (count_at x) 0 @@ Array.to_seq row) in
