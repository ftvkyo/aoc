...
let walk (m : char matrix) =
  let rec step ?blocked (visited : state list) x y d =
    if List.exists (( = ) (x, y, d)) visited then (
      ...
      print_endline
      @@ Printf.sprintf "blocked: %d,%d\n"
           (fst @@ Option.get blocked)
           (snd @@ Option.get blocked);
      [ Option.get blocked ])
    else
      let visited = (x, y, d) :: visited in
      let nx, ny = next x y d in
      try
        let obstacle = '#' = m#get nx ny || Some (nx, ny) = blocked in
        let used_path =
          List.exists (fun (x, y, _d) -> x = nx && y = ny) visited
        in
      ...
      with _ -> []
  in
  let x, y = Option.get @@ m#findi is_dir in
  let blocks = step [] x y @@ m#get x y in
