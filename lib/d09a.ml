...
let rec checksum_file id position length =
  if length <> 0 then
    (id * position) + checksum_file id (position + 1) (length - 1)
  else 0

let checksum (s : string) =
  printf "input: %s\n" s;
  let slen = String.length s in
  let rec checksum l_index l_position ?l_remaining_space r_index
      r_blocks_consumed =
    if l_index >= slen then (
      ...
      if l_index mod 2 = 0 then (
        if (* It's a file, consume entirely *)
          ...
          printf
            "Note: using up the %d remaining blocks of file id %d with index %d\n"
          ...
          checksum_file l_id l_position length
          + checksum (l_index + 1) (l_position + length) r_index
              r_blocks_consumed)
      else (
        ...
        if l_index >= r_index then (
          ...
          | None ->
              ...
              checksum l_index l_position ~l_remaining_space:length r_index
                r_blocks_consumed
          | Some l_remaining_space ->
              ...
              if r_blocks_remaining > 0 then (
                let blocks_consuming =
                  min l_remaining_space r_blocks_remaining
                in
                printf
                  "    which will be filled with %d blocks from file id %d \
                   with index %d that had %d blocks remaining\n"
                ...
                + checksum l_index
                    (l_position + blocks_consuming)
                    ~l_remaining_space r_index r_blocks_consumed)
              else (
                printf
                  "    but the file id %d with index %d has been used up \
                   completely, so we will start consuming another file\n\
                  \  retry: " r_id r_index;
