...
let rec checksum_of ~at (id : file_id) (length : length) =
  if length <> 0 then (id * at) + checksum_of ~at:(at + 1) id (length - 1) else 0
  ...
  object (self)
    ...
    method checksum () : int =
      let f (acc_pos, acc_sum) (frag : fragment) =
        ...
        | File (id, length) -> acc_pos + length, acc_sum + checksum_of id length ~at:acc_pos
      in
    ...
    method str () : string =
      let f acc (frag : fragment) =
        ...
        | File (id, length) -> acc ^ String.concat "" @@ List.init length (fun _ -> sprintf "[%d]" id)
      in
          ...
          let rec update ?(moved = false) into =
            ...
            | (File (head_file_id, head_file_length) as head) :: tail ->
                if file_id = head_file_id then
                  ...
                  if moved then (* Remove the file from this position as it was moved. *)
                    Free head_file_length :: tail
                  else (* Keep the file here as we failed to move it. *)
                    head :: tail
                else (* It's just a file, keep intact. *)
            ...
            | (Free free_space as head) :: tail ->
                if (not moved) && free_space >= file_length then
                  ...
                  let newtail = update ~moved:true @@ (Free (free_space - file_length) :: tail) in
                  frag :: newtail
